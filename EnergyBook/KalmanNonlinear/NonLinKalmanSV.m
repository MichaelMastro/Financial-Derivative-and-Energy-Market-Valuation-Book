...
% NonLinKalmanSV calls unscented transform and particle 
% filter based on Heston stochastic variance process 
...
param=zeros(5,1);
    ...
    param(4)=ThetaGuess; 
    param(5)=RhoGuess
    options=optimset('MaxIter',1,'Display','iter');
    % Optimized already for data 
    % Increase MaxIter if data changes
    
%%%%%   Pick Routine %%%%  
code='PFHeston' %'UTHeston'%  
%%%%%%%%%
...
if (code=='PFHeston') % Extra plot to show particles
    ...
    for j=1:NumPart
        for i=(1:len)
         plot(Time(i),PartVec(i,j), '.',...
          'MarkerFaceColor','g', 'MarkerSize', 1+2*WeightVec(i,j))
         hold on
        end
    end
    hold on
